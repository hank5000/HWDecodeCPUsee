...
public class MainActivity extends Activity implements VideoToFrames.Callback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        s = sv.getHolder().getSurface();
//        hack = new Hack();
//        hack.setSurfaceBufferCount(s,1);
    ...
    NativeRender nativeRender = null;
    Hack hack = null;
    public void onFrameReady(ByteBuffer b, int width, int height) {
        if(b.isDirect()) {
            nativeRender.renderingNV21ToSurface2(s, b, width, height, width * height * 3 / 2);
        } else {
            byte[] bs = new byte[b.remaining()];
            b.get(bs);
            nativeRender.renderingNV21ToSurface(s, bs, width, height, width * height * 3 / 2);
        }
    }
