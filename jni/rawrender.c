...
//rendering
JNIEXPORT void JNICALL Java_com_via_utility_NativeRender_renderingNV21ToSurface
...
//rendering
JNIEXPORT void JNICALL Java_com_via_utility_NativeRender_renderingNV21ToSurface2
(JNIEnv * env, jobject thiz,jobject javaSurface,jbyteArray pixelsArray,jint w,jint h,jint size) {
	ANativeWindow* window = ANativeWindow_fromSurface(env, javaSurface);
	if(window!=NULL)
	{
		...
		ANativeWindow_Buffer buffer;
		if (ANativeWindow_lock(window, &buffer, NULL) == 0) {
			unsigned char *pixels = (unsigned char*)(*env)->GetDirectBufferAddress(env,pixelsArray);
